...
func importCmd() (int, error) {
	...
	var format, siteFlag, date, tyme, datetime string
...
func importLog(fp io.ReadCloser, url, key, file, format, date, tyme, datetime string, follow, silent bool) error {
	...
	go persistLog(hits, url, key, silent, follow)
	for {
		line, err := scan.Line()
		if err == io.EOF {
			break
...
func persistLog(hits <-chan handlers.APICountRequestHit, url, key string, silent, follow bool) {
	var (
		collect = make([]handlers.APICountRequestHit, 0, 100)
		mu      sync.Mutex
	)
	persist := func() {
		mu.Lock()
		...
		cp := make([]handlers.APICountRequestHit, len(collect))
		copy(cp, collect)
		collect = make([]handlers.APICountRequestHit, 0, 100)
		mu.Unlock()
		...
		go func() {
			...
			err := importSend(url, key, silent, follow, cp)
			if err != nil {
				zlog.Error(err)
			}
		}()
